...
func openstackTerraformStateMigration(input *go_hook.HookInput, dc dependency.Container) error {
	input.LogEntry.Infof("Starting Terraform migrate hook")
	...
	for _, secret := range terraformStateSecrets.Items {
		...
		secretBackupExists, err := SecretBackupExists(backupSecretName, TerraformStateNamespace, kubeCl, input)
		...
		input.LogEntry.Infof("Secret backup with name %s/%s doesn't exist! Continue migration for Secret/%s/%s", TerraformStateNamespace, backupSecretName, TerraformStateNamespace, secret.ObjectMeta.Name)
		...
		input.LogEntry.Infof("Found old resources. Starting migration for Secret/%s/%s", TerraformStateNamespace, secret.ObjectMeta.Name)
...
func SecretBackupExists(backupSecretName string, namespace string, kubeCl k8s.Client, input *go_hook.HookInput) (bool, error) {
	input.LogEntry.Debugf("Function SecretBackupExists: Starting function with parameters: backupSecretName=%s; namespace=%s", backupSecretName, namespace)
	backupSecret, err := kubeCl.CoreV1().
		Secrets(namespace).
		Get(context.TODO(), backupSecretName, metav1.GetOptions{})
	input.LogEntry.Debugf("Function SecretBackupExists: Get secret. Secret=%s; err=%s", backupSecret, err)

	if errors.IsNotFound(err) {
		input.LogEntry.Debugf("Function SecretBackupExists: errors.IsNotFound(err) = %t. secret \"%s\" not found'. Return false and nil", errors.IsNotFound(err), backupSecretName)
		return false, nil
	}

	if err == nil {
		input.LogEntry.Debugf("Function SecretBackupExists: err == nil. Return true and nil")
		return true, nil
	}

	input.LogEntry.Debugf("Function SecretBackupExists: err !=nil and errors.IsNotFound(err) no true. Return false and err")
	return false, err
}
